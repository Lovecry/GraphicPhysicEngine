...
void * RenderToTexture::operator new(size_t nSize, size_t nAlignment)
...
void RenderToTexture::operator delete(void *pObject, size_t nAlignment)
...
bool RenderToTexture::Initialize(ID3D11Device* device, int textureWidth, int textureHeight, float screenDepth, float screenNear)
{
	...
	m_projectionMatrix = DirectX::XMMatrixPerspectiveFovLH(((float)DirectX::XM_PI / 4.0f), ((float)textureWidth / (float)textureHeight), screenNear, screenDepth);

	// Create an orthographic projection matrix for 2D rendering.
	m_orthoMatrix = DirectX::XMMatrixOrthographicLH((float)textureWidth, (float)textureHeight, screenNear, screenDepth);
	return true;
}
