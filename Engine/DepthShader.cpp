...
bool DepthShaderClass::InitializeShader(ID3D11Device* device, HWND hwnd, WCHAR* vsFilename, WCHAR* psFilename)
{
	...
	ID3DBlob* vs = nullptr;
	result = utility::ShaderLoader::vertexPreBuiltLoad(L"./DepthVertexShader.cso", device, &m_vertexShader, &vs);
	if (FAILED(result))
	{
	...
	result = utility::ShaderLoader::pixelPreBuiltLoad(L"./DepthPixelShader.cso", device, &m_pixelShader, &ps);
	if (FAILED(result))
	{
		// If the shader failed to compile it should have writen something to the error message.
		if (errorMessage)
	...
	result = utility::ShaderLoader::createInputLayout(L"./DepthVertexShader.cso", device, polygonLayout, numElements, &m_layout);
	if (FAILED(result))
	{
		return false;
	}
	...
	matrixBufferDesc.ByteWidth = sizeof(utility::Transformations);
