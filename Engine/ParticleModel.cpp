...
ParticleModel::ParticleModel(TextureManager* textureManager)
{
...
ParticleModel::ParticleModel(const ParticleModel& other)
{
}
...
bool ParticleModel::Initialize(ID3D11Device* device)
{
	bool result;
	...
	m_texture = m_TextureManager->GetTexture(Constants::STAR);
...
bool ParticleModel::Frame(float frameTime, ID3D11DeviceContext* deviceContext)
{
	bool result;
...
int ParticleModel::GetIndexCount()
{
	return m_indexCount;
}


bool ParticleModel::InitializeParticleSystem()
{
	int i;
...
void ParticleModel::ShutdownParticleSystem()
{
...
bool ParticleModel::InitializeBuffers(ID3D11Device* device)
{
...
void ParticleModel::EmitParticles(float frameTime)
{
...
void ParticleModel::UpdateParticles(float frameTime)
{
	int i;
...
void ParticleModel::KillParticles()
{
	int i, j;
...
bool ParticleModel::UpdateBuffers(ID3D11DeviceContext* deviceContext)
{
	int index, i;
	HRESULT result;
	...
	{
		// Bottom left.
		m_vertices[index].position = DirectX::XMFLOAT3(m_particleList[i].positionX - m_particleSize, m_particleList[i].positionY - m_particleSize, m_particleList[i].positionZ);
		m_vertices[index].texture = DirectX::XMFLOAT2(0.0f, 1.0f);
